...
public class GetUserConnectionsQueryHandler : IRequestHandler<GetUserConnectionsQuery, GetUserConnectionsResult>
{
    ...
    {
        _logger.LogInfo("查詢用戶連線歷史", new { request.UserId, request.Skip, request.Take });
        ...
        try
        {
            ...
            _logger.LogInfo("查詢用戶連線歷史完成", new { request.UserId, ConnectionCount = connections.Count(), totalCount });
