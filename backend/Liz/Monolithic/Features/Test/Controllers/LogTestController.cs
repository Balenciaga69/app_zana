...
public class LogTestController : ControllerBase
{
    ...
    {
        // 測試一般資訊日誌
        _appLogger.LogInfo("測試 Serilog 配置", new { TestData = "這是測試資料", Timestamp = DateTime.UtcNow });

        // 測試警告日誌
        _appLogger.LogWarn("這是警告測試", new { WarningLevel = "Medium", Component = "LogTestController" });
    ...
    public IActionResult TestErrorLogging()
    {
        ...
        {
            _appLogger.LogError("測試錯誤日誌記錄", ex, new { ErrorContext = "LogTestController.TestErrorLogging", UserId = "test-user-123" });

            return BadRequest(new { Message = "錯誤已記錄到日誌中", ErrorId = Activity.Current?.Id ?? "unknown" });
        }
    }
}
